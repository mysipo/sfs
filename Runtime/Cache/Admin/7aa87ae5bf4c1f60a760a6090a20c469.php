...
<script>
	...
	<div class="cf">
		<a class="btn" href="<?php echo U('video_add',array('video_id'=>$video_id));?>">添加视频</a>
		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($list["node_id"]); ?></td>
						<td><a href="http://qzonestyle.gtimg.cn/open/qcloud/video/flash/video_player.swf?swfv=beta1&auto_play=0&file_id=<?php echo ($list["qcloud_id"]); ?>&app_id=1251979435&version=1" target="_black" title="<?php echo ($list["subtitle"]); ?>"><?php echo ($list["node_title"]); ?></a></td>
						<td><?php echo ($list["qcloud_id"]); ?></td>
                        <td><?php echo ($list["qcloud_app"]); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('video_edit?id='.$list['node_id']);?>">编辑</a>
							<a class="confirm ajax-get" title="删除" href="<?php echo U('video_del?id='.$list['node_id']);?>">删除</a>
						</td>
