...
<script>
    ...
    <div class="tools">
        <a class="btn" href="<?php echo U('Playlive/add');?>">新 增</a>
        <button class="btn ajax-post" target-form="ids" url="<?php echo U('Playlive/setStatus',array('status'=>1));?>">启 用</button>
        <button class="btn ajax-post" target-form="ids" url="<?php echo U('Playlive/setStatus',array('status'=>0));?>">禁 用</button>
        <a class="btn" href="<?php echo U('Playlive/generate');?>">删除</a>
    </div>
...
<table class="">
	...
	<?php if(!empty($list)): ?><tbody>
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$live): $mod = ($i % 2 );++$i;?><tr>
				<td><input class="ids row-selected" type="checkbox" name="ids[]" id="<?php echo ($live["live_id"]); ?>" value="<?php echo ($live['live_id']); ?>"> </td>
				<td><?php echo ($live["live_id"]); ?></td>
				<td><a href="<?php echo U('Serise/index?live_id='.$live['live_id']);?>"><?php echo ($live["live_title"]); ?></a></td>
				<td><?php echo ($live["tch_id"]); ?>/<?php echo ($live["tch_name"]); ?></td>
				<td><?php echo ($live["tutor_id"]); ?>/<?php echo ($live["tutor_name"]); ?></td>
				<td><?php echo ($live["live_start_time"]); ?></td>
				<td><?php echo ($live["live_stop_time"]); ?></td>
				<td>
					<a title="编辑" href="<?php echo U('edit?id='.$live['live_id']);?>">编辑</a>
					<a href="<?php echo U('playlive/setstatus?ids='.$live['live_id'].'&status='.abs(1-$vo['status']));?>" class="ajax-get"><?php echo (show_status_op($list["status"])); ?></a>
