...
<script>
	...
	<div class="data-table table-striped">
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($class["cl_id"]); ?></td>
						<td><?php echo ($class["cl_title"]); ?></td>
						<td><?php echo ($class["series_num"]); ?></td>
                        <td><?php echo ($class["theme_num"]); ?></td>
                        <td><?php echo ($class["live_num"]); ?></td>
                        <td><?php echo ($class["video_num"]); ?></td>
                        <!-- <td width="200">
                            <form action="<?php echo U('edit');?>" method="post">
                                <div class="order"><input type="text" name="cl_sort" class="input-mini" value="<?php echo ($class["cl_sort"]); ?>" onkeyup="if(/\D/.test(this.value)){this.value='';}" maxlength="4"></div>
                                <input type="hidden" name="cl_id" value="<?php echo ($class["cl_id"]); ?>">
                                <input type="hidden" name="cl_title" value="<?php echo ($class["cl_title"]); ?>">
                                <span class="help-inline msg" style="line-height:24px;margin-left:5px;color:#635c73;"></span>
                            </form>

                        </td> -->
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$class['cl_id']);?>">编辑</a>
							<!-- <a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$class['cl_id']);?>">删除</a> -->
						</td>
