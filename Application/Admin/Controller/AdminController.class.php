...
class AdminController extends Controller {
    ...
    protected function getMenu(){
        ...
        $cate       =   M('Category')->where(array('status'=>1))->field('id,title,pid,allow_publish,url,name')->order('pid,sort')->select();
                ...
                foreach ($value['_child'] as $ka=>&$va){
                    if(!empty($va['_child'])){
                        foreach ($va['_child'] as $k=>&$v){
                            $v['pid']   =   $va['id'];
                            $is_child = false;
                            if(strcasecmp($va['name'], CONTROLLER_NAME)==0)
                                $is_child = true;
                        }
                    }
                    //展开子分类的父分类
                    if(strcasecmp($va['name'], CONTROLLER_NAME)==0 || $is_child){
                        $is_child = false;
                        if($hide_cate){
                            $value['current']   =   true;
                            $va['current']      =   true;
    ...
    protected function gensee($subject='',$time='',$uri=false){
        if($time=='' || empty($time)){
        ...
        // 参数数组
        $data = array (
                'loginName' => C('GENSEE_USER'),
                'password'  => C('GENSEE_PASS'),
                'subject'   =>  $subject,
                'startDate' =>  $time,
                'scene'     => 0
        );
