...
<script>
	...
	<div class="data-table">
        ...
        <table class="">
            ...
            <tbody>
        	<tbody>
        		<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><tr>
        				<td><?php echo ($t["t_id"]); ?></td>
        				<td><?php echo ($t["t_title"]); ?></td>
                        <td><?php echo ($t["cl_title"]); ?></td>
                        <td><a title="编辑" href="<?php echo U('add_live?id='.$t['t_id']);?>">课程</a>|<a title="编辑" href="<?php echo U('add_series?id='.$t['t_id']);?>">系列</a></td>
                        <td><?php echo ($t["username"]); ?></td>
        				<td><?php echo (substr($t["t_time"],0,10)); ?></td>
        				<td>
                            <a title="编辑" href="<?php echo U('edit?id='.$t['t_id']);?>">编辑</a>
        					<a class="confirm ajax-get" title="删除" href="<?php echo U('del?id='.$t['t_id']);?>">删除</a>
        				</td>
    ...
    (function($){
        $(".data-table")
            ...
            $('#cl_id').change(function(){
                ...
                window.location.href="<?php echo U('Theme/index');?>&cl_id="+p1+"";//页面跳转并传参
            })
        })(jQuery);
</script>
